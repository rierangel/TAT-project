...
from .models import Noticia, HaciendoDiferencia, Congreso , CulturaTributaria

from .serializer import NoticiaSerializers,ViewNoticiaSerializers, CongresoSerializers, ViewCongresoSerializers, HaciendoDiferenciaSerializers, ViewHaciendoDiferenciaSerializers, CulturaTributariaSerializers, ViewCulturaTributariaSerializers
...
class CulturaTributariasListView(APIView, PaginationHandlerMixinApiView):
    """
    url: http://localhost:8000/publicaciones/resoluciones/
    """
    queryset = CulturaTributaria.objects.all().order_by("-fecha")
    serializer_class = CulturaTributariaSerializers
    pagination_class = CustomPagination
...
class CulturaTributariaView(APIView):
    """
    url: http://127.0.0.1:8000/noticias/titulo
    """
    def get(self, request, slug):
        # queryset = get_object_or_404(Noticia, slug=slug )
        queryset = CulturaTributaria.objects.filter(slug=slug)
        serializer = ViewCulturaTributariaSerializers(queryset, many=True)
