...
export default function QueryLayer(Component, data, buscador, url, name) {
  ...
  return function () {
    ...
    const [query, setQuery] = useState({
      search: "",
      year: "",
      // previous: newData.links.previous && newData.links.previous ,
    })
    ...
    useEffect(() => {
      if(newData.links.next){
        setQuery({ ...query, ["next"]: newData.links.next }) 
      }
      if(newData.links.previous){
        setQuery({ ...query, ["previous"]: newData.links.previous }) 
      }
      console.log(query);
    }, [newData])
      ...
      {newData.links &&
        <ul className="pagination rounded-lg flex justify-center text my-9">
          ...
          {newData.links.previous &&
            <li onClick={() => console.log(newData.links)} className="border1 rounded-l-lg">
          ...
          {newData.links.next &&
            <li onClick={() => handlePage(newData.links.next) } className="border1 rounded-r-lg">
              <svg width="13" height="12" viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
