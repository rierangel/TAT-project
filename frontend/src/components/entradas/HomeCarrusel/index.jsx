...
export default function Index({ data }) {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            controler[i].className ="opacity-40"

        }
        main[current].className = main[current].className.replace("hidden", "");
        controler[current].classList.toggle("opacity-40") 
    ...
    const handleClickControler =(key)=>{
        console.log("key");
        setCurrent(key)
    }
                ...
                <div className={styles.menu} ref={controlersRef}>
                        {data.map((e, i) => (
                            <div className='opacity-40' key={i}   >
                                <img src={e.img} alt="" onClick={() => handleClickControler(i)} />
                            </div>
                        ))}            
                </div>
